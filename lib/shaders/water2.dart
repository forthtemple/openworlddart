...
class Water extends THREE.Mesh {
  ...
  Future<void> _init(Map<String,dynamic> options)async{
		final color = options['color'] ?? 0xFFFFFF;// THREE.Color(0xFFFFFF).fromHex(options['color'] ?? 0xFFFFFF);
		...
		final flowDirection = options['flowDirection'] ?? THREE.Vector2( 1, 0 );
		...
		final normalMap0 = options['normalMap0'] ?? await textureLoader.loadAsync( 'assets/textures/water/Water_1_M_Normal.jpg');
		final normalMap1 = options['normalMap1'] ?? await textureLoader.loadAsync( 'assets/textures/water/Water_2_M_Normal.jpg');

    halfCycle = cycle * 0.5;

    var reflector;
    if (reflectivity>0) {
      reflector = Reflector(geometry, {
